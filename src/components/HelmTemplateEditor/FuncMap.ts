...
export class FuncCompletionMeta implements CompletionMeta {

  constructor(name: string, args: string, doc: string) {
    this.name = name
    this.args = args
    this.doc = doc
  }
...
`),
  ...
  public releaseVals(): CompletionMeta[] {
    return [
      ...
      this.v("ServiceName", "Service", "Name of the service that produced the release (almost always Tiller)"),
  ...
  public filesVals(): CompletionMeta[] {
    return [
      this.f("Get", "Get $path", "Get file contents. Path is relative to chart."),
      this.f("GetBytes", "GetBytes $path", "Get file contents as a byte array. Path is relative to chart.")
    ];
  }

  public capabilitiesVals(): CompletionMeta[] {
    return [
      this.v("KubeVersion", "KubeVersion", "Kubernetes version"),
      this.v("TillerVersion", "TillerVersion", "Tiller version"),
      this.f("ApiVersions.Has", `ApiVersions.Has "batch/v1"`, "Returns true if the given Kubernetes API/version is present on the cluster")
    ];
  }

  public chartVals(): CompletionMeta[] {
    return [
  ...
  public helmFuncs(): FuncCompletionMeta[] {
    return [
  ...
  public builtinFuncs(): FuncCompletionMeta[] {
    return [
  ...
  public sprigFuncs(): FuncCompletionMeta[] {
    return [
      // 2.12.0
  ...
  public f(name: string, args: string, doc: string): FuncCompletionMeta {
    return new FuncCompletionMeta(name, args, doc)
  }

  public v(name: string, use: string, doc: string): ConstantCompletionMeta {
